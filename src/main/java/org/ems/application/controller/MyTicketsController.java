...
import org.ems.application.service.ticket.TicketLoaderService;
import org.ems.application.service.ticket.TicketFilterService;
import org.ems.application.service.ticket.TicketCalculationService;
import org.ems.application.service.ticket.TicketQRCodeService;
import org.ems.application.service.ticket.TicketExportService;
...
public class MyTicketsController {
    ...
    public void initialize() {
        ...
        try {
            ...
            // TODO: Enable after project rebuild
            // qrCodeConverterService = new TicketQRCodeConverterService();
    ...
    private void loadMyTicketsAsync() {
        ...
        AsyncTaskService.runAsync(
                ...
                tickets -> {
                    ...
                    // Convert QR codes from DB format to unique display format
                    // TODO: Enable after project rebuild
                    // long qrConvertStart = System.currentTimeMillis();
                    // System.out.println("    🔄 Converting QR codes for display...");
                    // if (qrCodeConverterService != null) {
                    //     result = qrCodeConverterService.convertDisplayDTOsQRCodes(result);
                    //     System.out.println("  ✓ QR codes converted in " + (System.currentTimeMillis() - qrConvertStart) + "ms");
                    // }
    ...
    public void onViewQRCode() {
        ...
        try {
            ...
            // QR Code Image Section - Generate and display PNG image
            System.out.println("🎟️ Generating QR code image from data: " + selected.getQrCode().substring(0, Math.min(50, selected.getQrCode().length())) + "...");
            byte[] qrCodeImage = ticketQRCodeService.generateQRCodeImage(selected.getQrCode());

            if (qrCodeImage != null && qrCodeImage.length > 0) {
                try {
                    // Create ImageView for QR code PNG
                    ...
                    // QR Code container with label and instructions
                    ...
                    Label qrHintLabel = new Label("✓ Ready to scan");
                    qrHintLabel.setStyle("-fx-font-size: 11; -fx-text-fill: #27ae60;");

                    qrBox.getChildren().addAll(qrTitleLabel, qrImageView, qrHintLabel);
                    content.getChildren().add(qrBox);

                    System.out.println("✓ QR code image (PNG) generated and displayed successfully");
                } catch (Exception e) {
                    System.err.println("✗ Failed to display QR code image: " + e.getMessage());
                    e.printStackTrace();
                    addQRCodeTextFallback(content, selected.getQrCode(), "Image Generation Failed");
                }
            } else {
                System.out.println("⚠️ QR code image generation returned null or empty");
                addQRCodeTextFallback(content, selected.getQrCode(), "Failed to Generate Image");
            }
