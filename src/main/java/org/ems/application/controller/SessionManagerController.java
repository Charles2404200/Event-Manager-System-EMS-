...
import org.ems.application.service.session.SessionCacheManager;
...
public class SessionManagerController {
    ...
    @FXML private Label detailMaterialLabel;
    @FXML private Button viewMaterialButton;
    ...
    private void displaySessionDetails(Session session) {
        ...
        try {
            ...
            // Display material link if available
            long materialStart = System.currentTimeMillis();
            String materialPath = session.getMaterialPath();
            if (materialPath != null && !materialPath.isEmpty()) {
                detailMaterialLabel.setText(materialPath);
                viewMaterialButton.setDisable(false);
                System.out.println("  ✓ Material link updated in " + (System.currentTimeMillis() - materialStart) + " ms");
            } else {
                detailMaterialLabel.setText("No material uploaded");
                viewMaterialButton.setDisable(true);
                System.out.println("  ✓ Material link cleared in " + (System.currentTimeMillis() - materialStart) + " ms");
            }
    ...
    @FXML
    public void onViewMaterial() {
        if (selectedSession == null) {
            showWarning("No Selection", "Please select a session first!");
            return;
        }

        String materialPath = selectedSession.getMaterialPath();
        if (materialPath == null || materialPath.isEmpty()) {
            showWarning("No Material", "No material uploaded for this session!");
            return;
        }

        System.out.println("📥 [onViewMaterial] Opening material: " + materialPath);

        try {
            // Check if it's a URL (S3/Cloudflare link)
            if (materialPath.startsWith("http://") || materialPath.startsWith("https://")) {
                System.out.println("  🌐 Opening URL: " + materialPath);
                // Open URL in browser
                java.awt.Desktop.getDesktop().browse(new java.net.URI(materialPath));
            } else {
                // Local file
                System.out.println("  📁 Opening local file: " + materialPath);
                java.io.File file = new java.io.File(materialPath);
                if (file.exists()) {
                    java.awt.Desktop.getDesktop().open(file);
                } else {
                    showError("File Not Found", "The material file does not exist at:\n" + materialPath);
                }
            }
        } catch (Exception e) {
            System.err.println("  ✗ Error opening material: " + e.getMessage());
            showError("Error Opening Material", "Cannot open material:\n" + e.getMessage());
        }
    }
