 ...
 * Delegates image generation to TicketQRCodeImageService
...
public class TicketQRCodeService {
    ...
    public TicketQRCodeService() {
        this.imageService = new TicketQRCodeImageService();
    }

    /**
     * Generate QR code image bytes from QR code data
     * Supports both DB format (qr-12345) and base64 encoded format
     * @param qrCodeData The QR code data string
     * @return Image bytes (PNG), or null if generation fails
     */
    public byte[] generateQRCodeImage(String qrCodeData) {
        ...
        try {
            // Use new image service for better handling
